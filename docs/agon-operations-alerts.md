...
The `agon_operations_alerts` row is canonical. The in-app notification row and `agon_alert_delivery_outbox` row both reference that incident. Telegram retries use a leased durable outbox with bounded exponential backoff, so an auth-service restart does not lose delivery work.

## Recipient and authentication

`AGON_OPERATIONS_ALERT_OPERATOR_ADDRESS` identifies the operations account that receives both in-app and Telegram delivery. During migration it falls back to `AGON_CERTIFICATION_ALERT_OPERATOR_ADDRESS`.

The recipient must:

1. exist in the `operators` table;
2. link Telegram through the normal authenticated account flow;
3. remain the same address configured for certification and Arena operations.
...
4. Trigger the identical warning again and verify the occurrence count increases without a second delivery.
5. Escalate it to critical and verify one new Telegram escalation is delivered.
6. Stop Telegram delivery temporarily, raise an alert, restart delivery, and confirm the queued item is delivered.
7. Acknowledge a critical incident through the admin console and verify its status changes to `acknowledged`.
8. Confirm certification and Arena state are identical whether Telegram succeeds or fails.

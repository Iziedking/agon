...
import { ChallengeKind, ChallengeStatus } from "./types/ArcRunTypes.sol";
...
contract ChallengeArena is AccessControl, ReentrancyGuard {
    // ============ Roles ============
    ...
    function joinChallenge(uint256 id, uint256 agentId) external nonReentrant {
        Challenge storage ch = _challenges[id];
        if (ch.creator == address(0)) revert ChallengeDoesNotExist();
        if (ch.status != ChallengeStatus.OPEN) revert ChallengeNotOpen();
        // forge-lint: disable-next-line(block-timestamp)

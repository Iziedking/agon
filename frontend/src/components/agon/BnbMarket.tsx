...
export function BnbMarket({ view = "market" }: { view?: "market" | "detail" | "publish" | "playground" }) {
  ...
  return <div className="min-h-screen bg-canvas text-ink"><AppHeader /><main>
    <section className="relative mx-auto max-w-[1600px] px-4 pt-14 sm:px-6 sm:pt-16"><CornerMarkers /><SectionHeader headingAs="h1" eyebrow={`AGON MARKET / ${network.brand} ${network.environment}`} heading={view === "market" ? "FIND AN AGENT" : view === "publish" ? "LIST YOUR AGENT" : view === "playground" ? "AGENT PLAYGROUND" : "AGENT DETAILS"} subDeck={view === "market" ? `Discover agents on ${network.name}. Inspect the owner, service, and evidence before choosing.` : view === "playground" ? `Run read-only agent analysis on ${network.name}. Inspect the evidence before granting authority.` : `Identity, service, and proof scoped to ${network.name}.`} right={view === "market" ? <AgonAuthAction href="/market/new">LIST YOUR AGENT</AgonAuthAction> : <TagButton variant="ghost" href={networkHref("/market", networkKey)}>BACK TO MARKET</TagButton>} /></section>

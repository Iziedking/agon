...
export function BnbMarket({ view = "market" }: { view?: "market" | "detail" | "publish" | "playground" }) {
  ...
  const needSignIn = () => router.push(networkHref("/login", networkKey));
  return <div className="min-h-screen bg-canvas text-ink"><AppHeader /><main>
    {view === "market" || view === "publish" ? <section className="relative mx-auto max-w-[1600px] px-4 pt-14 sm:px-6 sm:pt-16"><CornerMarkers /><SectionHeader headingAs="h1" eyebrow={`AGON MARKET / ${network.brand} ${network.environment}`} heading={view === "market" ? "FIND AN AGENT" : "LIST YOUR AGENT"} subDeck={view === "market" ? `Discover agents on ${network.name}. Inspect the owner, service, and evidence before choosing.` : "List a service so buyers can discover and use it."} right={view === "market" ? <AgonAuthAction href="/market/new">LIST YOUR AGENT</AgonAuthAction> : <TagButton variant="ghost" href={networkHref("/market", networkKey)}>BACK TO MARKET</TagButton>} /></section> : <section className="mx-auto max-w-[1600px] px-4 pt-10 sm:px-6 sm:pt-12"><TagButton variant="ghost" href={networkHref("/market", networkKey)}>BACK TO MARKET</TagButton></section>}

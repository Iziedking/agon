  ...
  try {
    ...
    if (prior.rows[0]) {
      ...
      if (recoverable.rows[0]) {
        await connection.query("COMMIT");
        return view(recoverable.rows[0]);
      }
      // The daily cap is for real active jobs, not abandoned quote attempts.
      // A create transaction is included even if the browser lost the response

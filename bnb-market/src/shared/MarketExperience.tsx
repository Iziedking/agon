...
import { A2ARunPanel } from "./A2ARunPanel";
import { bnbHref, emptyCategoryCopy, isReportService, networkTruth, requestStatus, recoveryCopy } from "./marketplace/journey";
...
// Use the chain's own vocabulary. "BSC Testnet" is what the explorer, the
// wallet and the registry call chain 97, so the buyer can match it by eye.
const network = (chainId: BnbChain) => `BSC ${chainId === 97 ? "Testnet" : "Mainnet"}`;
const networkWithChain = (chainId: BnbChain) => `${network(chainId)} · chain ${chainId}`;
...
function AgentRow({ agent, chainId, readiness, category }: { agent: AgentSummary; chainId: BnbChain; readiness: LpHiringReadiness | null; category: Category }) {
  ...
  return <article className={PANEL}>
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_180px_180px]">
      ...
      <dl className="space-y-4"><Fact label="Price">{report && readiness?.priceDisplay ? `${readiness.priceDisplay} ${readiness.token?.symbol ?? ""} / report` : "Not published"}</Fact><Fact label="Network">{networkWithChain(chainId)}</Fact></dl>
...
export function MarketDiscovery({ chainId }: { chainId: BnbChain }) {
  ...
  const truth = networkTruth(chainId);

  return <div className="space-y-6">
    <section aria-labelledby="network-truth-heading" className={`${PANEL} !p-4 sm:!p-5`}>
      <p className={LABEL}><span className="text-accent">■</span> {truth.chainLabel}</p>
      <h2 id="network-truth-heading" className="mt-2 font-mono text-sm uppercase tracking-wide text-ink">{truth.headline}</h2>
      <p className={`${BODY} mt-2 max-w-[80ch]`}>{truth.detail}</p>
      {truth.switchTo ? <a className={`${BUTTON} mt-4`} href={bnbHref(truth.switchTo, "/market")}>Switch to BSC Testnet →</a> : null}
    </section>
    ...
    {!category ? <p className={BODY}>Choose one category to see agents that responded to a live connection check.</p> : <section aria-labelledby="choose-agent-heading" className="space-y-4">
      ...
      {busy ? <Notice>Checking responding {CATEGORY_GUIDANCE[category].label.toLowerCase()} agents on {truth.chainLabel}...</Notice> : null}
      ...
      {!busy && !error && !items.length ? (() => {
        const empty = emptyCategoryCopy(chainId, CATEGORY_GUIDANCE[category].label);
        return <Notice><h3 className="font-stencil text-2xl uppercase">{empty.heading}</h3><p className="mt-3">{empty.detail}</p><div className="mt-4 flex flex-wrap gap-2"><button className={BUTTON} onClick={() => setRetry((value) => value + 1)}>Check again</button>{empty.switchTo ? <a className={PRIMARY} href={bnbHref(empty.switchTo, `/market?category=${category}`)}>See this goal on BSC Testnet →</a> : null}</div></Notice>;
      })() : null}
...
function ServiceFacts({ agent, readiness }: { agent: AgentDetail; readiness: LpHiringReadiness | null }) {
  ...
  return <dl className="grid gap-5 sm:grid-cols-2">
    ...
    <Fact label="Network">{networkWithChain(agent.chainId)}</Fact><Fact label="Task evidence">No verified task result supplied</Fact>
...
export function AgentService({ chainId, id, signedIn = false, onNeedSignIn = () => undefined, walletRequest = null }: { chainId: BnbChain; id: string; signedIn?: boolean; onNeedSignIn?: () => void; walletRequest?: WalletRequest | null }) {
  ...
  // An A2A registration is a runnable endpoint, so a buyer can test this
  // service for free even when no paid activation exists. Paid hiring still
  // requires the provider's own price and delivery path.
  const runnable = !report && !!agent?.services.some((service) => service.name.toUpperCase().replace(/[-_\s]/g, "") === "A2A");
  return <div className="space-y-6">{error ? <Notice error><p>{error}</p><button className={`${BUTTON} mt-3`} onClick={() => setRetry(x=>x+1)}>Retry</button></Notice> : null}{!agent && !error ? <Notice>Loading agent and service version…</Notice> : null}{agent ? <>
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="min-w-0 space-y-6"><section className={PANEL}><h2 className="font-stencil text-3xl uppercase">{report ? "Liquidity position report" : "Service overview"}</h2><p className={`${BODY} mt-4`}>{report ? "Get a report for one PancakeSwap position, including its current range and a proposed range when enough market data is available. Nothing in the position is changed." : runnable ? "This provider registered a runnable A2A endpoint, so you can send it a task for free and read its answer. It has not published a price, so AGON cannot prepare a paid job for it yet." : "Review the provider’s stated service and connection details. This listing does not yet expose a supported activation or payment flow in AGON."}</p><dl className="mt-5 grid gap-5 sm:grid-cols-2"><Fact label="You provide">{report ? "PancakeSwap position ID" : runnable ? "A task written in your own words" : "Input requirements not published"}</Fact><Fact label="You receive">{report ? "A downloadable position report" : runnable ? "The agent’s own answer, free, with no payment" : "Output requirements not published"}</Fact></dl><div className="mt-6 flex flex-wrap gap-2">{report ? <a className={PRIMARY} href="#hire-agent">Configure report →</a> : runnable ? <a className={PRIMARY} href="#run-agent">Run this agent free →</a> : <button className={BUTTON} disabled>Activation unavailable</button>}<a className={BUTTON} href={bnbHref(chainId, `/agon/playground?agent=${id}`)}>Open Playground →</a></div></section>
    {report ? <LpHiringPanel key={`${chainId}:${id}:${signedIn}`} chainId={chainId} version={agent.versionHash ?? "unavailable"} signedIn={signedIn} onNeedSignIn={onNeedSignIn} walletRequest={walletRequest} /> : <>{runnable ? <div id="run-agent"><A2ARunPanel key={`${chainId}:${id}`} chainId={chainId} agentId={id} /></div> : null}<section className={PANEL}><h2 className="font-stencil text-2xl uppercase">Before paid activation</h2><p className={`${BODY} mt-3`}>This provider must publish a supported service version, input requirements, price and permissions before AGON can prepare a wallet action. A successful connection check alone does not enable hiring.</p><dl className="mt-5 grid gap-4 sm:grid-cols-2"><Fact label="Call allowlist">Not provided</Fact><Fact label="Spend cap and expiry">Not provided</Fact><Fact label="Session registration">Not provided</Fact><Fact label="Stop or revoke">No session controls available here</Fact></dl><a className={`${BUTTON} mt-5`} href={bnbHref(chainId, "/market")}>Find another agent →</a></section></>}

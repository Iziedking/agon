...
export function ServiceCard({
  ...
  return (
    <article className="relative rounded-sm border border-[color:var(--hairline)] bg-[color:var(--canvas-2)] p-4">
      ...
      {onToggleCompare ? (
        <label className="mb-3 inline-flex min-h-11 cursor-pointer items-center gap-2 text-xs uppercase tracking-[0.12em] text-[color:var(--ink-2)]">
          <input
            type="checkbox"
            checked={compareSelected}
            disabled={compareDisabled && !compareSelected}
            onChange={() => onToggleCompare(service.id)}

...
async function readAllAgents(): Promise<{ agents: AgentSummary[]; error: string | null }> {
  ...
  try {
    ...
    return { agents: [...new Map(pages.flatMap((page) => page.items).map((agent) => [agent.id, agent])).values()], error: safeError(error) };
  }
  return { agents: [...new Map(pages.flatMap((page) => page.items).map((agent) => [agent.id, agent])).values()], error: null };
}
...
try {
  ...
  const readiness = {
    ...
    status: catalogRead.error || gaps.length || endpointFailures.length || registrationFailures.length || !runtimeReady ? "incomplete_live_readiness" : "marketplace_readiness_passed",
    sendsTransactions: false,
    runsProviderTasks: false,
    walletsLoaded: false,
  };
  console.log(JSON.stringify(readiness, null, 2));
  if (readiness.status !== "marketplace_readiness_passed") process.exitCode = 1;
} catch (error) {
